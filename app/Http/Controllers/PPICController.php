...
class PPICController extends Controller
{
    ...
    public function index()
    {
        $sops = Sop::whereHas('document', function ($query) {
            $query->where('category_document', 'PPIC');  // pastikan ini nama kolom dan value-nya sesuai
        })->with([
        ...
        return view('document-quality-sop.ppic.index', compact('sops', 'sopCount', 'wiCount', 'formCount'));
    }
    ...
    public function contentListPartial()
    {
        ...
        return view('document-quality-sop.ppic.partials.content-list', compact('sops'));
    }
    ...
    public function storeSOP(StoreDocumentRequest $request)
    {
        if ($request->hasFile('file_document')) {
            ...
            $file->move(public_path('documents/ppic/sop'), $filename); // Menggunakan $path yang sudah mengandung 'documents/sop/'
        ...
        $document = Document::create([
            'category_document' => 'PPIC', // Bisa dinamis kalau diperlukan
        ]);
        ...
        if ($request->ajax() || $request->expectsJson()) {
            return response()->json([
                'success' => true,
                'message' => 'Dokumen SOP Bagian PPIC berhasil disimpan'
            ]);
        }
    }
    ...
    public function updateSOP(UpdateDocumentRequest $request, Sop $sop)
    {
        ...
        if ($request->hasFile('file_document')) {
            ...
            if ($activeHistory->file_document && file_exists(public_path('documents/ppic/' . $activeHistory->file_document))) {
                unlink(public_path('documents/ppic/' . $activeHistory->file_document));
            }
            ...
            $file->move(public_path('documents/ppic/sop'), $filename);
    ...
    public function destroySOP(Sop $sop)
    {
        ...
        foreach ($sop->wis as $wi) {
            ...
            foreach ($wi->forms as $form) {
                foreach ($form->histories as $formHistory) {
                    ...
                    if ($formHistory->file_document && file_exists(public_path('documents/ppic/' . $formHistory->file_document))) {
                        unlink(public_path('documents/ppic/' . $formHistory->file_document)); // Menggunakan unlink
                    }
                }
            ...
            foreach ($wi->histories as $wiHistory) {
                ...
                if ($wiHistory->file_document && file_exists(public_path('documents/ppic/' . $wiHistory->file_document))) {
                    unlink(public_path('documents/ppic/' . $wiHistory->file_document)); // Menggunakan unlink
                }
            }
            $wi->histories()->delete();
            $wi->delete();
        ...
        foreach ($sop->histories as $sopHistory) {
            ...
            if ($sopHistory->file_document && file_exists(public_path('documents/ppic/' . $sopHistory->file_document))) {
                unlink(public_path('documents/ppic/' . $sopHistory->file_document)); // Menggunakan unlink
            }
        }
    ...
    public function storeWI(StoreDocumentRequest $request, $id)
    {
        if ($request->hasFile('file_document')) {
            ...
            $file->move(public_path('documents/ppic/wi'), $filename);
        } else {
            $path = null;
        }
        ...
        if ($request->ajax() || $request->expectsJson()) {
            return response()->json([
                'success' => true,
                'message' => 'Dokumen WI Bagian ppic Management berhasil disimpan'
            ]);
        }
    }
    ...
    public function storeForm(StoreDocumentRequest $request, $id)
    {
        if ($request->hasFile('file_document')) {
            ...
            $file->move(public_path('documents/ppic/form'), $filename);
        } else {
            $path = null;
        }
    ...
    public function destroyForm(Form $form)
    {
        ...
        foreach ($form->histories as $history) {
            if ($history->file_document && file_exists(public_path('documents/ppic/' . $history->file_document))) {
                unlink(public_path('documents/ppic/' . $history->file_document)); // Menggunakan unlink
            }
        }
            ...
            return [
                ...
                'revisions' => $sop->histories->map(function ($rev) {
                    return [
                        ...
                        'file_document' => $rev->file_document ? asset('documents/ppic/' . ltrim($rev->file_document, '/')) : null,  // Perbaikan: menggunakan 'documents/' dan menghapus '/' dari awal file_path
                    ...
                    return [
                        ...
                        'revisions' => $wi->histories->map(function ($rev) {
                            return [
                                'revision_number' => $rev->revision_number,
                                'title_document' => $rev->title_document,
                                'file_document' => $rev->file_document ? asset('documents/ppic/' . ltrim($rev->file_document, '/')) : null,  // Perbaikan
                                'date_document' => $rev->date_document,
                                'time_document' => $rev->time_document,
                                'is_active' => $rev->is_active,
                            ];
