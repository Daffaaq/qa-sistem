...
class QAQCController extends Controller
{
    ...
    public function index()
    {
        $sops = Sop::whereHas('document', function ($query) {
            $query->where('category_document', 'QA QC');  // pastikan ini nama kolom dan value-nya sesuai
        })->with([
        ...
        return view('document-quality-sop.qa-qc.index', compact('sops', 'sopCount', 'wiCount', 'formCount'));
    }
    ...
    public function contentListPartial()
    {
        ...
        return view('document-quality-sop.qa-qc.partials.content-list', compact('sops'));
    }
    ...
    public function storeSOP(StoreDocumentRequest $request)
    {
        if ($request->hasFile('file_document')) {
            ...
            $file->move(public_path('documents/qa-qc/sop'), $filename); // Menggunakan $path yang sudah mengandung 'documents/sop/'
        ...
        $document = Document::create([
            'category_document' => 'QA QC', // Bisa dinamis kalau diperlukan
        ]);
        ...
        if ($request->ajax() || $request->expectsJson()) {
            return response()->json([
                'success' => true,
                'message' => 'Dokumen SOP Bagian QA QC berhasil disimpan'
            ]);
        }
    }
    ...
    public function updateSOP(UpdateDocumentRequest $request, Sop $sop)
    {
        ...
        if ($request->hasFile('file_document')) {
            ...
            if ($activeHistory->file_document && file_exists(public_path('documents/qa-qc/' . $activeHistory->file_document))) {
                unlink(public_path('documents/qa-qc/' . $activeHistory->file_document));
            }
            ...
            $file->move(public_path('documents/qa-qc/sop'), $filename);
    ...
    public function destroySOP(Sop $sop)
    {
        ...
        foreach ($sop->wis as $wi) {
            ...
            foreach ($wi->forms as $form) {
                foreach ($form->histories as $formHistory) {
                    ...
                    if ($formHistory->file_document && file_exists(public_path('documents/qa-qc/' . $formHistory->file_document))) {
                        unlink(public_path('documents/qa-qc/' . $formHistory->file_document)); // Menggunakan unlink
                    }
                }
            ...
            foreach ($wi->histories as $wiHistory) {
                ...
                if ($wiHistory->file_document && file_exists(public_path('documents/qa-qc/' . $wiHistory->file_document))) {
                    unlink(public_path('documents/qa-qc/' . $wiHistory->file_document)); // Menggunakan unlink
                }
            }
            $wi->histories()->delete();
            $wi->delete();
        ...
        foreach ($sop->histories as $sopHistory) {
            ...
            if ($sopHistory->file_document && file_exists(public_path('documents/qa-qc/' . $sopHistory->file_document))) {
                unlink(public_path('documents/qa-qc/' . $sopHistory->file_document)); // Menggunakan unlink
            }
        }
    ...
    public function storeWI(StoreDocumentRequest $request, $id)
    {
        if ($request->hasFile('file_document')) {
            ...
            $file->move(public_path('documents/qa-qc/wi'), $filename);
        } else {
            $path = null;
        }
        ...
        if ($request->ajax() || $request->expectsJson()) {
            return response()->json([
                'success' => true,
                'message' => 'Dokumen WI Bagian QA QC berhasil disimpan'
            ]);
        }
    }
    ...
    public function storeForm(StoreDocumentRequest $request, $id)
    {
        if ($request->hasFile('file_document')) {
            ...
            $file->move(public_path('documents/qa-qc/form'), $filename);
        } else {
            $path = null;
        }
    ...
    public function destroyForm(Form $form)
    {
        ...
        foreach ($form->histories as $history) {
            if ($history->file_document && file_exists(public_path('documents/qa-qc/' . $history->file_document))) {
                unlink(public_path('documents/qa-qc/' . $history->file_document)); // Menggunakan unlink
            }
        }
