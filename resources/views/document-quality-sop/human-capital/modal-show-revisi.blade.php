...
@push('scripts')
    <script>
        $(document).on('click', '#btnShowAllRevisions', function() {
            ...
            $.get("{{ route('human-capital.revisi-show') }}", function(response) {
        ...
        function renderHistories(histories, docType = 'sop', parentPrefixedId = null) {
            ...
            const baseRoute = {
                sop: "{{ route('human-capital.sop.set-active', ['id' => ':id']) }}",
                wi: "{{ route('human-capital.wi.set-active', ['id' => ':id']) }}",
                form: "{{ route('human-capital.form.set-active', ['id' => ':id']) }}"
            };
