        ...
        <div class="modal-content">
            <form method="POST" enctype="multipart/form-data" id="formReviseWI">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="modalReviseWILabel">Revisi Dokumen WI Human Capital</h5>
...
@push('scripts')
    <script>
        ...
        $(document).on('click', '.btn-revise-wi', function(e) {
            ...
            const actionUrl = `{{ route('human-capital.revise-wi', ':id') }}`.replace(':id', wiId);
            $('#formReviseWI').attr('action', actionUrl);

            // Ambil data judul & file aktif dari server
            $.get("{{ route('human-capital.revisi-wi', ':id') }}".replace(':id', wiId), function(data) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        // Refresh count & list
                        $.get("{{ route('human-capital.count-wi') }}", function(res) {
