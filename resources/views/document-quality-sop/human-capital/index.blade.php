...
@section('content')
        ...
        <div class="card bg-light-info shadow-none position-relative overflow-hidden mb-4">
            <div class="card-body px-4 py-3">
                <div class="d-flex justify-content-between align-items-center">
                    <h4 class="fw-semibold mb-0">Human Capital SOP</h4>
                    <ol class="breadcrumb border border-info px-3 py-2 rounded">
                        <li class="breadcrumb-item">
                            <a href="{{ route('dashboard') }}" class="text-muted">Dashboard</a>
                        </li>
                        <li class="breadcrumb-item">
                            <span class="active text-primary fw-semibold">Human Capital SOP</span>
                        </li>
                    </ol>
                </div>
            </div>
    ...
    @include('document-quality-sop.human-capital.modal-preview')
    @include('document-quality-sop.human-capital.modal-create-sop')
    @include('document-quality-sop.human-capital.modal-edit-sop')
    @include('document-quality-sop.human-capital.modal-revisi-sop')
    @include('document-quality-sop.human-capital.modal-create-wi')
    @include('document-quality-sop.human-capital.modal-edit-wi')
    @include('document-quality-sop.human-capital.modal-revisi-wi')
    @include('document-quality-sop.human-capital.modal-create-form')
    @include('document-quality-sop.human-capital.modal-edit-form')
    @include('document-quality-sop.human-capital.modal-revisi-form')
    @include('document-quality-sop.human-capital.modal-show-revisi')
@endsection
...
@push('scripts')
                ...
                error: function() {
                    $('#sop-rep-list-container').html(`
                        <div class="text-danger text-center">
                            ...
                            <button id="retryButton" class="btn btn-warning mt-3">Coba Lagi</button>
                        </div>
                    `);

                    // Tambahkan event listener untuk tombol retry
                    $('#retryButton').on('click', function() {
                        loadSopList(); // Panggil fungsi untuk me-load ulang data
                    });
                }
            });
        }
    ...
    <script>
        ...
        function refreshAllCounts() {
            $.get("{{ route('human-capital.count-all') }}", function(res) {
        ...
        $(document).on('click', '.btn-delete-sop', function(e) {
            ...
            const url = `{{ route('human-capital.destroy-sop', ['sop' => ':sopId']) }}`.replace(':sopId', id);
        ...
        $(document).on('click', '.btn-delete-wi', function(e) {
            e.preventDefault();
            const id = $(this).data('id');
            const url = `{{ route('human-capital.destroy-wi', ['wi' => ':wiId']) }}`.replace(':wiId', id);
        ...
        $(document).on('click', '.btn-delete-form', function(e) {
            e.preventDefault();
            const id = $(this).data('id');
            const url = `{{ route('human-capital.destroy-form', ['form' => ':formId']) }}`.replace(':formId', id);
