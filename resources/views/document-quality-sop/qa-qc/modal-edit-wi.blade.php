<!-- Modal Edit WI -->
<div class="modal fade" id="modalEditWI" tabindex="-1" aria-labelledby="modalEditWILabel" aria-hidden="true">
...
@push('scripts')
    <script>
        ...
        $(document).on('click', '.btn-edit-wi', function(e) {
            ...
            $.get("{{ route('qa-qc.edit-wi', ['wi' => ':wiId']) }}".replace(':wiId', wiId), function(data) {
                $('#edit_wi_id').val(wiId);
                ...
                const updateWiUrl = "{{ route('qa-qc.update-wi', ['wi' => ':wiId']) }}".replace(':wiId',
                    wiId);
                $('#formEditWI').attr('action', updateWiUrl);

                // Show modal
                $('#modalEditWI').modal('show');
            });
        });
        ...
        $('#modalEditWI').on('hidden.bs.modal', function() {
