...
@push('scripts')
    <script>
        $(document).on('click', '#btnShowAllRevisions', function() {
            ...
            $.get("{{ route('qa-qc.revisi-show') }}", function(response) {
        ...
        $(document).on('click', '.btn-preview-file', function() {
            // const fileUrl = `${window.BASE_URL}/${$(this).data('file')}`;
    ...
    `;
            ...
            // Memuat dan menampilkan PDF dengan PDF.js
            if (fileUrl.endsWith('.pdf')) {
                const loadingTask = pdfjsLib.getDocument(fileUrl);
                loadingTask.promise.then(function(pdf) {
                    const totalPages = pdf.numPages;

                    // Loop untuk memuat dan merender semua halaman
                    for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
                        pdf.getPage(pageNum).then(function(page) {
                            const scale = 1.0; // Bisa disesuaikan
                            ...
                            page.render(renderContext).promise.then(function() {
                                // Tambahkan canvas ke dalam container
