...
@push('scripts')
    <script>
        ...
        $(document).on('click', '.btn-revise-wi', function(e) {
            ...
            const actionUrl = `{{ route('management-representative.revise-wi', ':id') }}`.replace(':id', wiId);
            $('#formReviseWI').attr('action', actionUrl);

            // Ambil data judul & file aktif dari server
            $.get("{{ route('management-representative.revisi-wi', ':id') }}".replace(':id', wiId), function(data) {
