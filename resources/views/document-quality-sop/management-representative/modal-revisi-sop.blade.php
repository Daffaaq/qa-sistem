...
@push('scripts')
    <script>
        ...
        $(document).on('click', '.btn-revise-sop', function(e) {
            ...
            const actionUrl = `{{ route('management-representative.revise-sop', ':id') }}`.replace(':id', sopId);
            $('#formReviseSOP').attr('action', actionUrl);

            // Ambil data judul & file aktif dari server
            $.get("{{ route('management-representative.revisi-sop', ':id') }}".replace(':id', sopId), function(data) {
