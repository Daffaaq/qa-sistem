...
@push('scripts')
    <script>
        $(document).on('click', '#btnShowAllRevisions', function() {
            ...
            $.get("{{ route('maintanance.revisi-show') }}", function(response) {
        ...
        function renderHistories(histories, docType = 'sop', parentPrefixedId = null) {
            ...
            const baseRoute = {
                sop: "{{ route('maintanance.sop.set-active', ['id' => ':id']) }}",
                wi: "{{ route('maintanance.wi.set-active', ['id' => ':id']) }}",
                form: "{{ route('maintanance.form.set-active', ['id' => ':id']) }}"
            };
