        ...
        <div class="modal-content">
            <form method="POST" enctype="multipart/form-data" id="formReviseSOP">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="modalReviseSOPLabel">Revisi Dokumen SOP Management IRGA</h5>
...
@push('scripts')
    <script>
        ...
        $(document).on('click', '.btn-revise-sop', function(e) {
            ...
            const actionUrl = `{{ route('irga.revise-sop', ':id') }}`.replace(':id', sopId);
            $('#formReviseSOP').attr('action', actionUrl);

            // Ambil data judul & file aktif dari server
            $.get("{{ route('irga.revisi-sop', ':id') }}".replace(':id', sopId), function(data) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        // Refresh count & list
                        $.get("{{ route('irga.count-sop') }}", function(res) {
