...
@push('scripts')
    <script>
        $(document).on('click', '#btnShowAllRevisions', function() {
            ...
            $.get("{{ route('engineering.revisi-show') }}", function(response) {
        ...
        function renderHistories(histories, docType = 'sop', parentPrefixedId = null) {
            ...
            const baseRoute = {
                sop: "{{ route('engineering.sop.set-active', ['id' => ':id']) }}",
                wi: "{{ route('engineering.wi.set-active', ['id' => ':id']) }}",
                form: "{{ route('engineering.form.set-active', ['id' => ':id']) }}"
            };
