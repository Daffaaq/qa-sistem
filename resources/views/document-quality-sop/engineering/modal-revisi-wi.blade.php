        ...
        <div class="modal-content">
            <form method="POST" enctype="multipart/form-data" id="formReviseWI">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="modalReviseWILabel">Revisi Dokumen WI Engineering</h5>
...
@push('scripts')
    <script>
        ...
        $(document).on('click', '.btn-revise-wi', function(e) {
            ...
            const actionUrl = `{{ route('engineering.revise-wi', ':id') }}`.replace(':id', wiId);
            $('#formReviseWI').attr('action', actionUrl);

            // Ambil data judul & file aktif dari server
            $.get("{{ route('engineering.revisi-wi', ':id') }}".replace(':id', wiId), function(data) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        // Refresh count & list
                        $.get("{{ route('engineering.count-wi') }}", function(res) {
