...
<div class="modal fade" id="modalCreateSOPPPIC" tabindex="-1" aria-labelledby="modalCreateSOPRepLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="{{ route('ppic.store-sop') }}" method="POST" enctype="multipart/form-data"
                id="formCreateSOPPPIC">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title" id="modalCreateSOPRepLabel">Tambah Dokumen SOP PPIC
                    </h5>
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <small class="form-text text-muted">
                            Maksimal 255 karakter. Contoh: "SOP PPIC PT IPG Tahun 2025".
                        </small>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="category_document" id="category_document_sop_rep"
                            class="form-control" value="PPIC" readonly>
                        <small class="form-text text-muted">
...
@push('scripts')
    <script>
        $('#formCreateSOPPPIC').on('submit', function(e) {
            e.preventDefault();
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        $('#formCreateSOPPPIC')[0].reset();
                        $('#modalCreateSOPPPIC').modal('hide');
                        $.get("{{ route('ppic.count-sop') }}", function(res) {
                ...
                error: function(xhr) {
                    ...
                    if (xhr.status === 422) {
                        ...
                        // Reset error lama
                        $('#formCreateSOPPPIC').find('.form-control').removeClass('is-invalid');
                        $('#formCreateSOPPPIC').find('.invalid-feedback').text('');

                        // Tampilkan error baru
                        $.each(errors, function(field, messages) {
                            let input = $(`#formCreateSOPPPIC [name="${field}"]`);
        ...
        $('#modalCreateSOPPPIC').on('hidden.bs.modal', function() {
            const form = $('#formCreateSOPPPIC');
