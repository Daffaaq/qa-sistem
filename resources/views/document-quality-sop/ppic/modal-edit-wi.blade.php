<!-- Modal Edit WI -->
<div class="modal fade" id="modalEditWIPPIC" tabindex="-1" aria-labelledby="modalEditWILabel" aria-hidden="true">
...
@push('scripts')
    <script>
        ...
        $(document).on('click', '.btn-edit-wi', function(e) {
            ...
            $.get("{{ route('ppic.edit-wi', ['wi' => ':wiId']) }}".replace(':wiId', wiId),
                function(data) {
                    $('#edit_wi_id').val(wiId);
                    ...
                    const updateWiUrl = "{{ route('ppic.update-wi', ['wi' => ':wiId']) }}"
                        .replace(':wiId',
                            wiId);
                    $('#formEditWI').attr('action', updateWiUrl);

                    // Show modal
                    $('#modalEditWIPPIC').modal('show');
                });
        });
        ...
        $('#modalEditWIPPIC').on('hidden.bs.modal', function() {
