        ...
        <div class="modal-content">
            <form method="POST" enctype="multipart/form-data" id="formReviseSOP">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="modalReviseSOPLabel">Revisi Dokumen SOP PPIC</h5>
...
@push('scripts')
    <script>
        ...
        $(document).on('click', '.btn-revise-sop', function(e) {
            ...
            const actionUrl = `{{ route('ppic.revise-sop', ':id') }}`.replace(':id', sopId);
            $('#formReviseSOP').attr('action', actionUrl);

            // Ambil data judul & file aktif dari server
            $.get("{{ route('ppic.revisi-sop', ':id') }}".replace(':id', sopId), function(data) {
